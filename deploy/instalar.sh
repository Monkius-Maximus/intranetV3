#!/usr/bin/env bash
# =============================================================================
# Instalador da Intranet SEPLAG numa VM Ubuntu (Server 20.04+).
#
...
#   2. Cria o usuário de serviço "intranet" e instala o app em /opt/intranet.
#   3. npm ci --omit=dev (dependências de produção, reprodutíveis).
#   4. Grava /etc/intranet.env (porta; credenciais do 1º admin na 1ª execução).
...
#
# Depois de instalar, carregue as pessoas reais (com o serviço parado):
...
PORTA="${PORTA:-80}"
DESTINO=/opt/intranet
USUARIO=intranet
ENV_FILE=/etc/intranet.env
...
# 5. Serviço systemd -----------------------------------------------------------
log "Gravando /etc/systemd/system/intranet.service (porta $PORTA)"
cat > /etc/systemd/system/intranet.service <<EOF
[Unit]
Description=Intranet SEPLAG (Node/Express, porta $PORTA)
...
if [ "$TEM_SYSTEMD" -eq 1 ]; then
  ...
  log "(sem systemd: unit gravada; ative com 'systemctl enable --now intranet' numa VM real)"
fi

# 6. Backup diário --------------------------------------------------------------
log "Agendando backup diário (guarda os últimos 30)"
cat > /etc/cron.daily/intranet-backup <<EOF
#!/bin/sh
# Backup diário do banco da intranet (data/ -> backups/), mantendo 30.
cp "$DESTINO/data/intranet.json" "$DESTINO/backups/backup-\$(date +%Y%m%d).json" 2>/dev/null || exit 0
...
EOF
chmod +x /etc/cron.daily/intranet-backup
...
echo "  4. Logs:    journalctl -u intranet -f"
